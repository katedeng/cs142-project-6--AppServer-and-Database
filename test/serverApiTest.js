...
const userDetailProperties = ['first_name', 'last_name', '_id', 'location', 'description', 'occupation'];
    ...
    describe('test using model data', function(done) {
        it('webServer does not use model data', function(done) {
            fs.readFile('../webServer.js', function(err, data) {
                ...
                assert(!data.toString().match(regex), 'webServer still contains reference to cs142 models.');
                done();
            });
        });
    });
                        ...
                        function(response) {
                            ...
                            response.on('end', function() {
                                ...
                                _.forEach(real_photos, function(real_photo) {
                                    ...
                                    assert.strictEqual(matches.length, 1, ' looking for photo ' + real_photo.file_name);
                                    ...
                                    if (real_photo.comments) {
                                        ...
                                        _.forEach(real_photo.comments, function(real_comment) {
                                            ...
                                            assert.strictEqual(
                                                extraProps3.length,
                                                0,
                                                'comment user object has extra properties: ' + extraProps3
                                            );
